-- ============================================================
-- [session-board] 초기 스키마 생성
-- ============================================================
-- "user", generation 테이블은 프로필 팀 소유이므로 FK 제약을 걸지 않습니다.
-- 코드 레벨에서는 ProfilePort를 통해 참조 무결성을 보장합니다.
-- ============================================================
...
CREATE TABLE event_post (
    ...
    author_id     UUID        NOT NULL,  -- "user".id (프로필 팀 소유, FK 없음)
    generation_id INT         NOT NULL,  -- generation.id (프로필 팀 소유, FK 없음)
    type          TEXT        NOT NULL,
    title         TEXT        NOT NULL,
    body          TEXT,
...
CREATE TABLE "like" (
    id         UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id    UUID        NOT NULL,  -- "user".id (프로필 팀 소유, FK 없음)
    post_id    UUID        NOT NULL REFERENCES event_post(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE resource (
    id          UUID                PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id  UUID                NOT NULL REFERENCES session(id) ON DELETE CASCADE,
    uploader_id UUID                NOT NULL,  -- "user".id (프로필 팀 소유, FK 없음)
    type        TEXT                NOT NULL,
    name        TEXT                NOT NULL,
    url         TEXT                NOT NULL,
    size_label  TEXT,
