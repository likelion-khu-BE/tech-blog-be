 ...
 *
 * "어떤 멤버가 어떤 기술을 어느 정도 수준으로 다루는가"를 기록한다.
 *
 * [왜 중간 테이블인가?]
 * 멤버와 기술 스택도 다대다(N:M) 관계다.
 * 한 멤버는 Java, Spring, Docker 등 여러 기술을 가질 수 있고,
 * 하나의 기술(예: Java)은 여러 멤버가 동시에 보유할 수 있다.
 * 이 MemberTechStack이 "멤버 A는 Java를 숙련도 5로 사용한다"는 관계를 하나의 행으로 표현한다.
 *
 * [DB 테이블: member_tech_stack]
 */
@Entity
@Table(
    name = "member_tech_stack",
    // 같은 멤버가 같은 기술 스택을 중복 등록하지 못하도록 복합 유니크 제약을 건다.
    uniqueConstraints = @UniqueConstraint(name = "uq_member_tech_stack", columnNames = {"member_id", "tech_stack_id"})
)
@Getter
...
public class MemberTechStack {
  ...
  /**
   * 멤버에게 기술 스택을 등록할 때 사용하는 정적 팩토리 메서드.
   * (예: MemberTechStack.create(member, javaStack, 4))
   */
