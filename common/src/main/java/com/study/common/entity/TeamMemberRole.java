 ...
 * 팀원 역할 엔티티.
 *
 * 한 팀원(TeamMember)이 맡은 역할 분야(RoleInTeam)를 기록한다.
 * 한 팀원이 여러 역할을 가질 수 있으므로 TeamMember와 1:N 관계다.
 *
 * 예) 홍길동(팀원)이 backend와 infra 두 역할을 맡으면
 *     team_member_role 행이 2개 생긴다:
 *     → (team_member_id=1, role=backend)
 *     → (team_member_id=1, role=infra)
 *
 * [DB 테이블: team_member_role]
 */
@Entity
...
public class TeamMemberRole {
  ...
  /**
   * 팀원에게 역할을 부여할 때 사용하는 정적 팩토리 메서드.
   * TeamMember.create() 내부에서 역할 수만큼 반복 호출된다.
   */
