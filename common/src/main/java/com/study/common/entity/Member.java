...
/**
 * 멤버(사용자) 엔티티.
 * @DynamicUpdate : 수정 시 변경된 컬럼만 UPDATE 쿼리에 포함시킨다 (불필요한 쿼리 최소화).
 * @NoArgsConstructor(PROTECTED) : 파라미터 없는 기본 생성자를 protected로 만든다.
 *                                  외부에서 new Member()를 직접 못 하게 막고, create() 팩토리 메서드를 쓰도록 강제한다.
 */
@Entity
@DynamicUpdate
@Table(
...
public class Member {
  ...
  // @Enumerated(EnumType.STRING) : Enum 값을 DB에 숫자(0,1,2...)가 아닌 문자열("backend", "frontend"...)로 저장한다.
  ...
  /**
   * 멤버가 보유한 기술 스택 목록.
   * mappedBy = "member" : MemberTechStack 엔티티에 있는 'member' 필드와 매핑된다는 뜻.
   * cascade = ALL : 멤버가 저장/삭제될 때 연결된 기술 스택 정보도 함께 처리한다.
   * orphanRemoval = true : 리스트에서 제거하면 DB에서도 해당 행을 삭제한다.
   */
  @jakarta.persistence.OneToMany(mappedBy = "member", cascade = jakarta.persistence.CascadeType.ALL, orphanRemoval = true)
   ...
   *
   * new Member()를 직접 쓰는 대신 이 메서드를 통해야 한다.
   * 필수값(name, email, sessionType)만 받고, 나머지는 나중에 updateProfile()로 채운다.
   */
  ...
  /**
   * 멤버의 프로필 정보를 수정할 때 호출하는 메서드.
   * 학과, 프로필 이미지, GitHub URL, 추가 링크를 한 번에 업데이트한다.
   */
  ...
  /**
   * JPA가 DB에 INSERT(저장)하기 직전에 자동으로 호출하는 콜백 메서드.
   * createdAt과 updatedAt을 현재 시각으로 초기화한다.
   * 개발자가 직접 호출할 필요 없이 JPA가 알아서 실행한다.
   */
  @PrePersist
  void prePersist() {
  ...
  /**
   * JPA가 DB에 UPDATE(수정)하기 직전에 자동으로 호출하는 콜백 메서드.
