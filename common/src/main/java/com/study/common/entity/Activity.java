 ...
 * 멤버 활동 이력 엔티티.
 *
 * 멤버가 블로그 글 작성, Q&A 답변, 세션 발표 등 활동을 할 때마다
 * 그 기록을 이 테이블에 한 행씩 쌓는다.
 * 각 활동에는 점수(score)가 부여되어 기여도 랭킹 등에 활용된다.
 *
 * [예시 데이터]
 * member_id=1(홍길동), type=blog_post, score=10  → "홍길동이 블로그 글을 써서 10점 획득"
...
public class Activity {
   ...
   *
   * 예) Activity.create(홍길동, ActivityType.blog_post, 글UUID, "blog_post", 10)
   *     → "홍길동이 해당 블로그 글을 작성해 10점 획득"을 기록
   */
  public static Activity create(Member member, ActivityType type, Long referenceId, String referenceType, int score) {
