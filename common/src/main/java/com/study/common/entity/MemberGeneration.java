 ...
 * 멤버-기수 연결 엔티티 (중간 테이블).
 *
 * "어떤 멤버가 몇 기에 어떤 역할로 참여했는가"를 기록한다.
 *
 * [왜 중간 테이블이 필요한가?]
 * 멤버와 기수는 다대다(N:M) 관계이다.
 * 한 멤버는 여러 기수에 참여할 수 있고 (13기, 14기 운영진 등),
 * 한 기수에는 여러 멤버가 참여한다.
 * DB에서는 이런 다대다 관계를 직접 표현할 수 없어서,
 * 이 MemberGeneration 같은 중간 테이블을 두어 '멤버 id + 기수 id' 쌍으로 관계를 기록한다.
 *
 * [DB 테이블: member_generation]
 */
@Entity
...
public class MemberGeneration {
  ...
  /**
   * 특정 멤버를 특정 기수에 특정 역할로 연결하는 정적 팩토리 메서드.
   * (예: "홍길동을 13기 운영진으로 등록")
   */
