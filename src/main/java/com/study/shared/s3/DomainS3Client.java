 ...
 * 도메인별 S3 클라이언트.
 *
 * <p>버킷을 생성 시점에 고정하고 Rate Limit · 파일 검증을 내장한다.
 * 각 도메인 Config에서 {@code @Bean}으로 등록해 사용한다.
 *
 * <pre>
...
public final class DomainS3Client {
   ...
   *
   * <p>Rate Limit → 파일명(확장자·수) 검증 → URL 생성 순서를 보장한다.
   * presigned URL 엔드포인트에서 호출한다.
   *
   ...
   *
   * <p>Content-Type({@code image/*}) · 파일 크기 검증 → 실패 시 S3 파일 자동 삭제.
   * 게시글·자료 저장 직전에 호출한다.
   *
   ...
   * S3 파일 삭제.
   *
   * <p>키가 존재하지 않아도 예외를 던지지 않는다.
   * 게시글·자료 삭제 시 DB 삭제 전에 호출한다.
   */
